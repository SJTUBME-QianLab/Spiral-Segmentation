...
def k_fold_pre(filename, image_list_file, fold):  # 生成的是源文件的5-fold序号
    ...
    num_per_fold = round(num / fold)  # 每一份有几个数据
    list_range = range(0, num)
    randnum = random.sample(list_range, num)
    folds = []
    # 把数据随机分成5份，folds[i]里是第i个fold的id序号
    for i in range(fold):
        if i != fold - 1:
            folds.append(randnum[i * num_per_fold: (i + 1) * num_per_fold])  # 均是id
        else:
            folds.append(randnum[i * num_per_fold:])
    train_index_5 = []
    test_index_5 = []
    for i in range(fold):
        # training集
